...
### Grid restyle v2 — solid lines + full right-side grid (2026-06)
Per user: removed dotted/hyphen grid style -> SOLID thin lines (vertLines rgba(255,255,255,0.09), DOM horz rows same); grid + time labels now continue across the empty future area right of the live candle.
How: FUTURE_WS_BARS=300 whitespace points appended to series data (withWhitespace() at all 4 setData sites incl. loadOlder); one whitespace slot appended per new candle in the drain RAF loop; scrollToRealTime replaced by snapToLive() (manual setVisibleLogicalRange anchoring last REAL candle +8 bars, since baseIndex now ends at whitespace); running-candle updates use series.update(bar, true) (historicalUpdate — REQUIRED, otherwise 'Cannot update oldest data' runtime error since candle is no longer the series' last point).
Verified: testing agent iteration_61 PASS 7/7 incl. 33s zero-interaction candle-close stability on 5s tf (no leftward shift regression), pan/zoom clamp, jump-to-live, tf/pair switch.
